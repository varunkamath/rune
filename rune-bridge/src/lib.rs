...
use rune_core::{Config, RuneEngine, search::{SearchMode, SearchQuery}};
...
impl RuneBridge {
    ...
    pub async fn initialize(&self, config_json: String) -> Result<()> {
        ...
        let rust_config = Config {
            workspace_roots: config.workspace_roots.into_iter().map(PathBuf::from).collect(),
        ...
        let engine = RuneEngine::new(rust_config).await
    ...
    pub async fn search(&self, query_json: String) -> Result<String> {
        ...
        let mode = match query.mode.as_str() {
            ...
            _ => return Err(Error::from_reason(format!("Invalid search mode: {}", query.mode))),
        };
        ...
        let response = engine.search().search(rust_query).await
