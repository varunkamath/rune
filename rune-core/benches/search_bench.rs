...
fn benchmark_symbol_search(c: &mut Criterion) {
    ...
    rt.block_on(async {
        ...
        let symbol_queries = vec!["Point", "calculate", "test", "Error", "main"];

        for query_str in symbol_queries {
            group.bench_with_input(
                BenchmarkId::new("symbol", query_str),
                &query_str,
                |b, &query_str| {
                    b.iter(|| {
                        rt.block_on(async {
...
fn benchmark_search_with_filters(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();
    let mut group = c.benchmark_group("search/filters");
    ...
    rt.block_on(async {
        ...
        // Benchmark with file pattern filters
        group.bench_function("with_file_patterns", |b| {
            b.iter(|| {
                rt.block_on(async {
                    let query = SearchQuery {
                        ...
                        offset: 0,
                        repositories: None,
                        file_patterns: Some(vec!["*.rs".to_string(), "*.py".to_string()]),
                    };
        ...
        group.bench_function("with_pagination", |b| {
            b.iter(|| {
                rt.block_on(async {
                    let query = SearchQuery {
                        query: "function".to_string(),
    ...
    rt.block_on(async {
        ...
        group.bench_function("p50_p95_p99", |b| {
            b.iter(|| {
                rt.block_on(async {
                    let query = SearchQuery {
                        query: "return".to_string(), // Common keyword
                        mode: SearchMode::Literal,
                        limit: 20,
                        offset: 0,
                        repositories: None,
